...
class RSFNetModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(RSFNetModel, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        if hasattr(self.net_g, 'fix_backbone'):
            if self.net_g.fix_backbone:
                self.net_g.fix_parameters() 
        ...
        if train_opt.get('mask_opt2'):
            self.cri_mask2 = build_loss(train_opt['mask_opt2']).to(self.device)
        else:
            self.cri_mask2 = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_mask:
            try:
                try:
                    l_mask = self.cri_mask(self.mask, self.mask_gt)
                except:
                    l_mask = self.cri_mask(self.mask[:,:self.mask_gt.size(1),...], self.mask_gt)
            except:
                try:
                    l_mask = self.cri_mask(self.mask, self.lq)
                except:
                    l_mask = self.cri_mask(self.mask)

            weight = 1.0 if (current_iter // 5000)%2 == 1 else 1.0
            l_total += weight * l_mask
            loss_dict['l_mask'] = weight * l_mask
        if self.cri_mask2:
            try:
                l_mask2 = self.cri_mask2(self.mask[:,:self.mask_gt.size(1),...], self.mask_gt)
            except:
                l_mask2 = self.cri_mask2(self.mask)
            weight = 1.0 if (current_iter // 5000)%2 == 1 else 1.0
            l_total += weight * l_mask2
            loss_dict['l_mask2'] = weight * l_mask2
        # cache loss
        if self.cri_cache:
        ...
        # grad loss
        if self.cri_grad:
            l_grad, self.grad_output, self.grad_gt = self.cri_grad(self.output, self.gt)
            l_total += l_grad
        ...
        # calculate matrics to check if overfit happened.
        # only when 'calculate_tensor' is True.
        if 'val' in self.opt and self.opt['val'].get('calculate_tensor', False):
            for name, opt_ in self.opt['val']['metrics'].items():
                loss_dict[name] = calculate_metric(dict(img=(self.output.detach()+1)/2.,img2=(self.gt.detach()+1)/2.), opt_)
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                if hasattr(self, 'in_params'):
                    results = self.net_g_ema(self.lq, self.in_params)
                else:
                    results = self.net_g_ema(self.lq)
            ...
            with torch.no_grad():
                begin = time.time()
                if hasattr(self, 'in_params'):
                    results = self.net_g(self.lq, self.in_params)
                else:
                    results = self.net_g(self.lq)
                self.time_cost = time.time() - begin
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        calculate_tensor = self.opt['val'].get('calculate_tensor', False)
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if time_cost:
                ...
                if hasattr(self.net_g, "time_cost"):
                    total_time_cost_split.append(self.net_g.time_cost)

            visuals = self.get_current_visuals()
            sr_img = tensor2img([visuals['result']], min_max=(-1,1))
            if calculate_tensor:
                metric_data['img'] = (visuals['result']+1)/2.
                if 'gt' in visuals:
                    metric_data['img2'] = (visuals['gt']+1)/2.
                    del self.gt
            else:
            ...
            if save_mask:
                ...
                if 'mask' in visuals:
                    mask_groups = torch.split(visuals['mask'], 1, dim=1)
            ...
            if save_params and 'params' in visuals:
                if params_tensor_all is None:
                    params_tensor_all = torch.zeros((len(dataloader),visuals['params'].size(1),visuals['params'].size(2),visuals['params'].size(3)))
            ...
            if with_metrics:
                ...
                for name, opt_ in self.opt['val']['metrics'].items():
                    ...
                    if save_metrics_per_image:
                        self.metric_results_per_image[name].append( [val_data['lq_path'], tmp] ) 
            if use_pbar:
                pbar.update(1)
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name, tb_logger):
        ...
        for metric, value in self.metric_results.items():
            if isinstance(value, torch.Tensor):
                value = torch.mean(value).cpu().numpy().item()
    ...
    def get_current_visuals(self):
        ...
        if hasattr(self, 'mask'):
            out_dict['mask'] = self.mask.detach()
            if self.mask.size(1) > 20:
                out_dict['mask'] = torch.max(out_dict['mask'], dim=1, keepdim=True)[1].type(torch.float32)
                out_dict['mask'] = out_dict['mask'] / self.mask.size(1)
        if hasattr(self, 'mask_gt'):
            out_dict['mask_gt'] = self.mask_gt.detach()
            if self.mask.size(1) > 20:
                out_dict['mask_gt'] = out_dict['mask_gt'].type(torch.float32) / self.mask.size(1)
    ...
    @torch.no_grad()
    def inference(self, image, params=None):
        assert isinstance(image, torch.Tensor), "image must be torch.Tensor"
        image = (image - 0.5)/0.5
        if params is None:
            input_data = {"lq": image}
        else:
            input_data = {"lq": image, 'in_params': params}
        if hasattr(self, 'infer_params'):
            input_data = {"lq": image, "in_params": self.infer_params}
        else:
            input_data = {"lq": image} 
        ...
        if visuals.get('result') is not None:
            ...
            #return_dict.update( {'result': (image+1)/2.*0.5+0.5*(visuals['result'] + 1) / 2.} )
        if visuals.get('params') is not None:
            return_dict.update( {'params': visuals['params']} )
            if not hasattr(self, 'infer_params'):
                self.infer_params = visuals['params']
        #        print(self.infer_params)
                #self.infer_params[:,0:1,:,1] += 0.2
        #        self.infer_params[:,2:3,:,3] -= 0.3
        #        self.infer_params[:,2:3,:,1] += 0.3
