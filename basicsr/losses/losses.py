...
from .loss_util import weighted_loss, ImageProcessing 
...
class GradientLoss(nn.Module):
    ...
    def forward(self, x, y, loss=True, mask = None):
        ...
        grad_xy = F.conv2d(y, self.weight_x, stride=1, padding=1)
        grad_yy = F.conv2d(y, self.weight_y, stride=1, padding=1)
        gradient_y = torch.abs(grad_xy) + torch.abs(grad_yy)
        if loss:
            if mask is None:
                #return self.loss_weight * torch.mean(torch.abs(gradient_y-gradient_x)/(gradient_x+self.eps)), gradient_x, gradient_y
                return self.loss_weight * self.l1_loss(gradient_y, gradient_x), gradient_x, gradient_y
                #return self.loss_weight * self.l1_loss(gradient_y/(gradient_x+self.eps), gradient_x/(gradient_x+self.eps)), gradient_x, gradient_y
            else:
                #print(torch.min(mask), torch.max(mask))
                gradient_x = gradient_x * mask
                gradient_y = gradient_y * mask
                #print(torch.min(gradient_x), torch.max(gradient_x))
                #print(torch.min(gradient_y), torch.max(gradient_y))
                return self.loss_weight * self.l1_loss(gradient_x, gradient_y), gradient_x, gradient_y

        else:
            gmap = torch.abs(gradient_x - gradient_y)
            gmap = torch.mean(gmap, dim=1, keepdim=True)
            gmap = torch.cat([gmap]*3, dim=1)
            gmap = gmap / torch.max(gmap)
            gmap[gmap < 0.3] = 0.
            gmap[gmap > 0.5] = 0.
            return gmap
...
class EdgeSaliencyLoss(nn.Module):
    def __init__(self, device="cuda:0", loss_weight=[1.0,]*8, alpha_sal=0.7):
    ...
    def forward(self, y_pred, y_gt):
        y_preds = torch.split(y_pred, 1, 1)
        y_gts = torch.split(y_gt, 1, 1)
        total_loss = 0.0
        for i in range(len(y_gts)):
            ...
            total_loss += \
                self.loss_weight[i] * ( self.alpha_sal * sal_loss + (1 - self.alpha_sal) * edge_loss )
        return total_loss
...
class DiceLoss(nn.Module):
    ...
    def forward(self, input, target=None):
        if target is None:
            target = torch.where(input > torch.mean(input,dim=(2,3),keepdim=True), 1, 0)
        ...
        d = (2 * a) / (b + c)
        return torch.mean(self.loss_weight * (1-d))
...
class CrossDiceLoss(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(input.size(1)):
            input_i = input[:,i:i+1,...].contiguous().view(input.size()[0], -1)
            for j in range(i+1, input.size(1)):
                target_j = input[:,j:j+1,...].contiguous().view(input.size()[0], -1).float()
                a = torch.sum(input_i * target_j, 1)
                b = torch.sum(input_i * input_i, 1) + 0.001
                c = torch.sum(target_j * target_j, 1) + 0.001
                d = (2 * a) / (b + c)
                loss.append( torch.mean(self.loss_weight * d) )
        return sum(loss) / len(loss)

@LOSS_REGISTRY.register() 
class CrossEntropyLoss(nn.Module):
    def __init__(self, loss_weight=1.0, eps=1e-15):
    ...
    def forward(self, input, target):
        #target = int((input.size(1)-1) * (target+1)/2.)
        tmp = torch.sum(target != 133)
        return self.loss_weight * self.nll(torch.log(input+self.eps), target.squeeze().long())
